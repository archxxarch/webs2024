...
<?php
    ...
    if(!isset($_SESSION['memberID'])){
        ...
        if(empty($boardTitle) || empty($boardContents)){
            echo "<script>alert('제목과 게시글을 작성해주세요.');</script>";
            echo "<script>window.location.href = 'boardWrite.php';</script>";
        } else {
            $boardTitle = $connect -> real_escape_string($boardTitle);
            $boardContents = $connect -> real_escape_string($boardContents);
        
            $sql = "INSERT INTO Community(memberID, boardTitle, boardContents, boardView, regTime) VALUES('$memberID', '$boardTitle', '$boardContents', '$boardView', '$regTime')";
            $connect -> query($sql);
